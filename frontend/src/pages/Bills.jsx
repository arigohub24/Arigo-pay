...
const BillsPayment = () => {
  ...
  const billTypes = [
    ...
    { 
      id: 'electricity', 
      name: 'Electricity', 
      icon: <GiElectric className="text-blue-600" />,
    ...
    { 
      id: 'betting', 
      name: 'Betting', 
      icon: <FaGamepad className="text-blue-600" />,
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h1 className="text-2xl sm:text-3xl font-bold text-blue-800">Pay Bills</h1>
          <p className="text-gray-600 mt-2 text-sm">Quickly pay for airtime, data, electricity, betting and more</p>
        </motion.div>
        ...
        >
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-lg border border-blue-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
            />
          </div>

          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
            <h2 className="text-lg font-semibold text-blue-800">Select Bill Type</h2>
            {billType && (
              <motion.button 
                onClick={() => setBillType('')}
        ...
        {!billType && (
          ...
          >
            {filteredBillTypes.map((type) => (
              <motion.div
                key={type.id}
                className={`p-4 rounded-xl border bg-white cursor-pointer transition-all ${billType === type.id ? 'border-blue-500 ring-2 ring-blue-200' : 'border-blue-200 hover:border-blue-300'}`}
                onClick={() => setBillType(type.id)}
                whileHover={{ y: -5 }}
                whileTap={{ scale: 0.98 }}
              >
                <div className="flex items-center">
                  <div className="p-3 rounded-full bg-blue-50 mr-4">{type.icon}</div>
                  <div>
                    <h3 className="font-semibold text-blue-800 text-sm">{type.name}</h3>
          ...
          >
              ...
              <div className="mb-4 sm:mb-6">
                ...
                <AnimatePresence>
                  {showSavedBeneficiaries && (
                    <motion.div 
                      className="mt-2 grid grid-cols-1 gap-2 max-h-60 overflow-y-auto"
                    ...
                    >
                      {savedBeneficiaries
                        ...
                        .map(beneficiary => (
                          ...
                          >
                            <h4 className="font-medium text-blue-800 text-sm">{beneficiary.name}</h4>
                            <p className="text-xs text-gray-600">{beneficiary.details}</p>
                          </motion.div>
                        ))}
                    </motion.div>
                  )}
            ...
            <AnimatePresence>
              {paymentStatus === 'processing' && (
                <motion.div 
                  className="bg-blue-50 p-4 rounded-lg mb-4 sm:mb-6 flex items-center"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                >
                  <FaSpinner className="animate-spin h-5 w-5 text-blue-600 mr-3" />
                  <p className="text-blue-600 font-medium text-sm">Processing your payment...</p>
                </motion.div>
              )}
              {paymentStatus === 'success' && (
                <motion.div 
                  className="bg-blue-50 p-4 rounded-lg mb-4 sm:mb-6"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                >
                  <div className="flex items-center">
                    ...
                    <p className="text-blue-600 font-medium text-sm">Payment Successful!</p>
                  </div>
              ...
              {paymentStatus === 'failed' && (
                <motion.div 
                  className="bg-red-50 p-4 rounded-lg mb-4 sm:mb-6"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                >
                  <div className="flex items-center">
                    ...
                    <p className="text-red-600 font-medium text-sm">Payment Failed</p>
                  </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {networkProviders.map(provider => (
                        <motion.button
                          ...
                          className={`py-2 px-3 rounded-lg border text-sm font-medium ${formData.networkProvider === provider.id ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-blue-200 hover:bg-blue-50'}`}
                          whileHover={{ scale: 1.02 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          {provider.name}
                ...
                {(billType === 'airtime' || billType === 'data') && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`w-full p-3 border ${errors.phoneNumber ? 'border-red-500' : 'border-blue-200'} rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
                    />
                ...
                {billType === 'electricity' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`w-full p-3 border ${errors.meterNumber ? 'border-red-500' : 'border-blue-200'} rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
                    />
                    {errors.meterNumber && <p className="mt-1 text-xs text-red-600">{errors.meterNumber}</p>}
                  </div>
                )}
                ...
                {billType === 'betting' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`w-full p-3 border ${errors.bettingId ? 'border-red-500' : 'border-blue-200'} rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
                    />
                    {errors.bettingId && <p className="mt-1 text-xs text-red-600">{errors.bettingId}</p>}
                  </div>
                )}
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {dataPlans[formData.networkProvider]?.map(plan => (
                        <motion.button
                          ...
                          className={`p-3 border rounded-lg text-left ${formData.internetPlan === plan.id ? 'border-blue-500 bg-blue-50' : 'border-blue-200 hover:bg-blue-50'}`}
                          whileHover={{ scale: 1.02 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          <div className="font-medium text-sm">{plan.name.split(' - ')[0]}</div>
                          <div className="text-blue-600 font-semibold text-sm">{plan.name.split(' - ')[1]}</div>
                        </motion.button>
                      ))}
                    </div>
                    {errors.internetPlan && <p className="mt-1 text-xs text-red-600">{errors.internetPlan}</p>}
                  </div>
                )}
                ...
                {billType === 'cable' && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {cablePlans.map(plan => (
                        <motion.button
                          ...
                          className={`p-3 border rounded-lg text-left ${formData.cablePlan === plan.id ? 'border-blue-500 bg-blue-50' : 'border-blue-200 hover:bg-blue-50'}`}
                          whileHover={{ scale: 1.02 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          <div className="font-medium text-sm">{plan.name.split(' - ')[0]}</div>
                    ...
                    {errors.cablePlan && <p className="mt-1 text-xs text-red-600">{errors.cablePlan}</p>}
                ...
                {billType === 'water' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`w-full p-3 border ${errors.waterAccount ? 'border-red-500' : 'border-blue-200'} rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
                    />
                    {errors.waterAccount && <p className="mt-1 text-xs text-red-600">{errors.waterAccount}</p>}
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className={`w-full pl-10 p-3 border ${errors.amount ? 'border-red-500' : 'border-blue-200'} rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
                      />
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {[100, 200, 500, 1000, 2000].map(amt => (
                          <motion.button
                            ...
                            className={`px-3 py-1 text-xs rounded-full ${formData.amount == amt ? 'bg-blue-100 text-blue-700 border border-blue-300' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'}`}
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                          >
                            ₦{amt.toLocaleString()}
              ...
              <motion.button
                type="submit"
                className="w-full mt-4 sm:mt-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold shadow-md text-sm sm:text-base"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
                Continue to Payment
        ...
        <AnimatePresence>
          {showPinModal && (
            ...
            >
              <motion.div 
                className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-sm shadow-2xl"
                initial={{ scale: 0.9, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.9, opacity: 0 }}
              >
                <h3 className="text-lg sm:text-xl font-semibold text-blue-800 mb-4">Confirm Payment</h3>
                
                <div className="bg-blue-50 p-4 rounded-lg mb-4 sm:mb-6">
                  <div className="flex justify-between mb-2">
                    <span className="text-gray-600 text-sm">Bill Type:</span>
                    <span className="font-medium text-sm">{billTypes.find(t => t.id === billType)?.name}</span>
                  </div>
                  {formData.phoneNumber && (
                    <div className="flex justify-between mb-2">
                      <span className="text-gray-600 text-sm">Phone Number:</span>
                      <span className="font-medium text-sm">{formData.phoneNumber}</span>
                    </div>
                  )}
                  {formData.meterNumber && (
                    <div className="flex justify-between mb-2">
                      <span className="text-gray-600 text-sm">Meter Number:</span>
                      <span className="font-medium text-sm">{formData.meterNumber}</span>
                    </div>
                  )}
                  {formData.bettingId && (
                    <div className="flex justify-between mb-2">
                      <span className="text-gray-600 text-sm">Betting ID:</span>
                      <span className="font-medium text-sm">{formData.bettingId}</span>
                    </div>
                  )}
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    className={`w-full p-3 border ${errors.pin ? 'border-red-500' : 'border-blue-200'} rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-center text-lg sm:text-xl tracking-widest`}
                    autoFocus
                  />
                ...
                <div className="flex justify-end space-x-3">
                  <motion.button
                    ...
                    className="px-4 py-2 border border-blue-200 rounded-lg text-gray-700 hover:bg-blue-50 text-sm"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    Cancel
